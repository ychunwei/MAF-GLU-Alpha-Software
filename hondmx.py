...
class DMXConnection(object):
  ...
  def setChannel(self, chan, val, autorender = False):
    ...
    if not 1 <= chan-1 <= DMX_SIZE:
      print ('Invalid channel specified: %d' % (chan-1))
      return
    # clamp value
  ...
  def render(self):
    ...
    # apparently encoding of last character is a turkish char, so used special encoding instead of UTF-8
    self.com.write(''.join(packet).encode('cp1254'))
